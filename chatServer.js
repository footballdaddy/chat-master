...
var findCommand = function (command, messageData) {
        var message;
        var messageDataParseLetters = messageData.toLowerCase().replace(/\s/g, '');
        var messageDataParseNumbers = messageDataParseLetters.match(/\d+/g);
        console.log(messageDataParseNumbers);
        var x,y;
        console.log(messageDataParseNumbers[0]);
        if (messageDataParseNumbers !== null)
        {
        x = messageDataParseNumbers[0];
        y = messageDataParseNumbers[1];
                console.log(messageDataParseNumbers[1]);
        ...
        switch (command) {
            case '/chat':
                ...
                break;
            case '/bot-say':
                if (x <= 9000 && y <= 9000 || !isNaN(x) && !isNaN(y))
                message = 'Chat bot ' + x + y;
                else 
                message = 'Unvalid entries for x, y corrdinates';
...
io.sockets.on('connection', function(socket) {
    ...
    socket.on('newMessage', function(data) {
        ...
        var split = data.msg.indexOf(' ');
        var char0 = data.msg.charAt(0);
        ...
        if (split === -1 || char0 !== '/') {
            ...
            command = messageData.substr(0, split);
            messageData.substr(split);

            messageData =  findCommand(command, messageData);
        };
        
                var message = {'room':data.room, 'username':obj.username, 'msg':messageData, 'command':command,  'date':new Date()};
